...
describe("verifyMercadoPagoSignature", () => {
  ...
  it("should sign the exact canonical id:...;request-id:...;ts:...;", () => {
    ...
    const wrongV1 = crypto.createHmac("sha256", SECRET).update(wrongOrder).digest("hex");
  ...
  it("should reject when the secret is wrong", () => {
    const result = verifyMercadoPagoSignature({
      signatureHeader: makeSignature({ dataId: "pay-123", xRequestId: "req-abc-123", ts: TS, secret: "wrong-secret" }),
      xRequestId: "req-abc-123",
      dataId: "pay-123",
      secret: SECRET,
      now: NOW,
  ...
  it("should omit empty parts from the canonical", () => {
    ...
    const v1WithoutId = crypto.createHmac("sha256", SECRET).update(canonicalWithoutId).digest("hex");
    ...
    const v1WithoutRequestId = crypto.createHmac("sha256", SECRET).update(canonicalWithoutRequestId).digest("hex");
