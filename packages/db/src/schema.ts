...
export const dbProducts = pgTable(
  'products',
  {
    ...
    tenantId: uuid('tenantId')
      .notNull()
      .references(() => dbTenants.id, { onDelete: 'restrict' }),
    categoryId: uuid('categoryId').references(() => dbCategories.id, {
      onDelete: 'set null',
    }),
    name: text('name').notNull(),
  ...
  (table) => {
    return {
      tenantSlugUnique: uniqueIndex('products_tenant_slug_idx').on(
        table.tenantId,
        table.slug,
      ),
...
export const dbProductImages = pgTable(
  'product_images',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    productId: uuid('productId')
      .notNull()
      .references(() => dbProducts.id, { onDelete: 'cascade' }),
    tenantId: uuid('tenantId')
      .notNull()
...
export const dbProductVariants = pgTable(
  'product_variants',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenantId')
  ...
  (table) => {
    return {
      tenantSkuUnique: uniqueIndex('product_variants_tenant_sku_idx').on(
        table.tenantId,
        table.sku,
      ),
...
export const dbCustomers = pgTable(
  'customers',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenantId')
  ...
  (table) => {
    return {
      tenantEmailUnique: uniqueIndex('customers_tenant_email_idx').on(
        table.tenantId,
        table.email,
      ),
...
export const dbAdminUsers = pgTable(
  'admin_users',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    tenantId: uuid('tenantId').references(() => dbTenants.id, {
      onDelete: 'set null',
    }),
    createdAt: timestamp('createdAt', { withTimezone: true })
      .defaultNow()
...
export const dbOrders = pgTable(
  'orders',
  {
    ...
    tenantId: uuid('tenantId')
      .notNull()
      .references(() => dbTenants.id, { onDelete: 'restrict' }),
    customerId: uuid('customerId').references(() => dbCustomers.id, {
      onDelete: 'set null',
    }),
...
export const dbOrderItems = pgTable(
  'order_items',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenantId')
    ...
    orderId: uuid('orderId')
      .notNull()
      .references(() => dbOrders.id, { onDelete: 'cascade' }),
    productVariantId: uuid('productVariantId')
      .notNull()
      .references(() => dbProductVariants.id, { onDelete: 'restrict' }),
...
export const dbCategories = pgTable(
  'categories',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenantId')
  ...
  (table) => {
    return {
      tenantSlugUnique: uniqueIndex('categories_tenant_slug_idx').on(
        table.tenantId,
        table.slug,
      ),
...
export const dbShippingMethods = pgTable(
  'shipping_methods',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenantId')
