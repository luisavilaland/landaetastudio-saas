...
test.describe('Webhook MercadoPago - firma', () => {
  ...
  test.beforeAll(async () => {
    ...
    const tenants = await sql<
      { id: string }[]
    >`SELECT "id" FROM tenants WHERE slug = 'tienda1' LIMIT 1`
    if (tenants.length === 0) {
      await sql.end()
  ...
  test('firma válida + pago aprobado → orden pasa a confirmed', async ({
    request,
  }) => {
    const orderId = orderIds[0]
    const ts = Math.floor(Date.now() / 1000)
    const signature = makeSignature({
      dataId: MAGIC_APPROVED,
      ts,
