...
describe("PUT /api/config/tenant/domain", () => {
  ...
  it("should return 400 when customDomain is invalid", async () => {
    ...
    const response = await PUT(mockReq("PUT", { customDomain: "http://mitienda.com" }));
    const body = await response.json();
  ...
  it("should update domain successfully", async () => {
    ...
    chain.limit.mockResolvedValueOnce([{ id: "tenant-1", slug: "test", customDomain: "mitienda.com" }]);
    (db as any).returning = vi.fn().mockResolvedValue([{ id: "tenant-1", slug: "test", customDomain: "mitienda.com" }]);
  ...
  it("should clear domain when empty string", async () => {
    ...
    (db as any).returning = vi.fn().mockResolvedValue([{ id: "tenant-1", slug: "test", customDomain: null }]);
