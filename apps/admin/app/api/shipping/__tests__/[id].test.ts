...
async function handlePut(
  ...
  return NextResponse.json({
    method: { ...method, ...updates, updatedAt: new Date() },
  })
}
...
describe('GET /api/shipping/[id]', () => {
  ...
  it('should only return methods belonging to tenant', async () => {
    ...
    const noMatch =
      otherTenantMethod.tenantId !== 'tenant-1' ? null : otherTenantMethod
...
describe('PUT /api/shipping/[id]', () => {
  ...
  it('should return 404 when method not found', async () => {
    const res = await handlePut({ user: { tenantId: 'tenant-1' } }, null, {
      name: 'Nuevo',
    })
    expect(res.status).toBe(404)
  })
