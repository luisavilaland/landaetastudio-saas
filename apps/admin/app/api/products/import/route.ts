...
export async function POST(request: NextRequest) {
  try {
    ...
    const categoryBySlug = categories.reduce(
      (acc, cat) => {
        acc[cat.slug] = cat.id
        return acc
      },
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (isNaN(price) || price < 0) {
        results.push({
          ...
          name: row.name,
          status: 'error',
          reason: 'Precio inválido (debe ser número >= 0)',
        })
        continue
      }
      ...
      if (isNaN(stock) || stock < 0) {
        results.push({
          ...
          name: row.name,
          status: 'error',
          reason: 'Stock inválido (debe ser número >= 0)',
        })
        continue
      }
      ...
      try {
        await withTenantContext(tenantId, async (tx) => {
          ...
          const existing = await tx
            ...
            .from(dbProducts)
            .where(
              and(eq(dbProducts.slug, slug), eq(dbProducts.tenantId, tenantId)),
            )
            .limit(1)
          ...
          if (row.category_slug?.trim()) {
            ...
            if (!categoryId) {
              throw new Error(
                `SKIPPED:Categoría '${row.category_slug}' no encontrada`,
              )
            }
          }
