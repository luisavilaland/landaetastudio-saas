  ...
  try {
    ...
    return await withTenantContext(tenantId, async (tx) => {
      ...
      const variant = await tx
        ...
        .where(
          and(
            eq(dbProductVariants.productId, id),
  ...
  try {
    ...
    const readResult = await withTenantContext(tenantId, async (tx) => {
        ...
        for (const variant of existingVariants) {
          ...
          if (newSku !== variant.sku) {
            ...
            if (existingSku.length > 0) {
              return {
                ok: false as const,
                error: `El SKU generado "${newSku}" ya existe en otra variante del mismo producto`,
                field: 'sku' as const,
                status: 409,
              }
            }
      ...
      if (categoryId !== undefined)
        baseUpdateFields.categoryId = categoryId || null
      ...
      if (existingVariants.length > 0) {
        ...
        if (slugChanged) {
          const newSlug = normalizedSlug ?? product[0].slug
          const options =
            (currentVariant.options as Record<string, string>) || {}
          const optionValues = Object.values(options)
          const newSku = `${newSlug}-${optionValues.join('-').toLowerCase()}`
        ...
        if (skuFromBody && skuFromBody !== currentVariant.sku) {
          ...
          logger.debug(
            { from: currentVariant.sku, to: newSku },
            '[PUT Product] SKU changing',
          )
          ...
          logger.debug(
            { sku: skuFromBody },
            '[PUT Product] SKU unchanged, skipping update',
          )
        } else {
          logger.debug(
            { sku: currentVariant.sku },
            '[PUT Product] No SKU sent, keeping current',
          )
        }
        ...
        logger.debug(
          { productId: id },
          '[PUT Product] No variant found, creating new variant',
        )
      ...
      return {
        ...
        existingVariants,
        normalizedSlug,
        slugChanged: normalizedSlug
          ? normalizedSlug !== product[0].slug
          : false,
        now,
        safeSlug,
    ...
    if (!readResult.ok) {
      ...
      return NextResponse.json(
        { error: err.error, field: err.field },
        { status: err.status },
      )
    }
    ...
    if (image && image.size > 0) {
      try {
        ...
        newImageUrl = await uploadImage(
          buffer,
          `products/${Date.now()}-${safeSlug}.${ext}`,
          image.type,
        )
      } catch (uploadError) {
        logger.error(
          { error: uploadError },
          '[PUT Product] Error uploading image',
        )
        return NextResponse.json(
      ...
      try {
        ...
        logger.error(
          { error: deleteError },
          '[PUT Product] Error deleting image',
        )
      }
    }
    ...
    return await withTenantContext(tenantId, async (tx) => {
      if (Object.keys(baseUpdateFields).length > 1) {
        logger.debug(
          { fields: baseUpdateFields },
          '[PUT Product] Updating product fields',
        )
        await tx
          .update(dbProducts)
      ...
      if (slugChanged && existingVariants.length > 0) {
        ...
        for (const variant of existingVariants) {
          ...
          if (newSku !== variant.sku) {
            await tx
              ...
              .where(
                and(
                  eq(dbProductVariants.id, variant.id),
                  eq(dbProductVariants.tenantId, tenantId),
                ),
              )
          }
        ...
        logger.debug(
          { fields: variantFields },
          '[PUT Product] Inserting new variant',
        )
        await tx.insert(dbProductVariants).values(variantFields as any)
      }
      ...
      if (updatedProduct.length === 0) {
        logger.error(
          ...
          '[PUT Product] Product deleted during update',
        )
        return NextResponse.json(
          { error: 'Producto eliminado durante la actualización' },
          { status: 409 },
        )
      }
    ...
    logger.error(
      { error, productId: id, tenantId },
      '[PUT Product] Failed to update product',
    )
    ...
    if (
      ...
      return NextResponse.json(
        { error: 'El SKU ya existe en otra variante', field: 'sku' },
        { status: 409 },
      )
    }
