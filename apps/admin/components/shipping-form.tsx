...
export function ShippingForm({ mode = "create", initialData }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Error al guardar método de envío");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="freeShippingThreshold" className="block text-sm font-medium text-zinc-700">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="estimatedDaysMin" className="block text-sm font-medium text-zinc-700">
          ...
          <div>
            <label htmlFor="estimatedDaysMax" className="block text-sm font-medium text-zinc-700">
        ...
        <div className="flex items-center">
          <input
            ...
            className="h-4 w-4 text-zinc-900 focus:ring-zinc-500 border-zinc-300 rounded"
          />
          <label htmlFor="isActive" className="ml-2 block text-sm text-zinc-700">
            Activo
          </label>
        </div>
