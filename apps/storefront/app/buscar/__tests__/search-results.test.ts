...
describe("SearchResults (server component)", () => {
  it("debe consultar dentro de withTenantContext con el tenantId resuelto", async () => {
    vi.mocked(withTenantContext).mockImplementation(async (tenantId: string, cb: (tx: any) => any) =>
      cb(txMock([
        [
          { id: "p1", name: "Remera", slug: "remera", description: null, imageUrl: null, status: "active", createdAt: new Date() },
        ],
        [{ count: 1 }],
        [{ id: "v1", productId: "p1", price: 1000, stock: 3, sku: "REM-1" }],
        [{ id: "i1", productId: "p1", url: "https://img", alt: null, position: 0 }],
      ]))
    );

    const results = (await SearchResults({ query: "remera", offset: 0 })) as React.ReactElement;
  ...
  it("debe manejar búsqueda sin resultados sin construir IN () — ni variants ni images", async () => {
    ...
    const results = (await SearchResults({ query: "zzz", offset: 0 })) as React.ReactElement;
