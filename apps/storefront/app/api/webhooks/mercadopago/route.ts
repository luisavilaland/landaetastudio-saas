...
export async function POST(request: NextRequest) {
  try {
    ...
    const simMode =
      ...
      process.env.E2E_WEBHOOK_TEST === '1'
    ...
    const externalRef: string | null =
      mockExternalRef || payment.external_reference
    ...
    if (refParts.length !== 2) {
      logger.error(
        { externalRef },
        'external_reference sin tenantId — orden pre-deploy, reconciliar manualmente',
      )
    ...
    logger.debug(
      { orderId, tenantId },
      'Order ID and tenant from external_reference',
    )
    ...
    if (!newStatus) {
      logger.debug(
        { paymentId, paymentStatus: payment.status },
    ...
    if (newStatus === 'confirmed') {
      const emailData = await withTenantContext(tenantId, async (tx) => {
        ...
        return {
          ...
          total: order.total,
          shippingName:
            (order.shippingDetails as { name?: string } | null)?.name ||
            'Cliente',
        }
      })
      ...
      await withTenantContext(tenantId, async (tx) => {
        ...
        const orderItems = await tx
          ...
          .where(
            and(
              eq(dbOrderItems.orderId, orderId),
        ...
        for (const item of orderItems) {
          const [variant] = await tx
            ...
            .where(
              and(
                eq(dbProductVariants.id, item.productVariantId),
