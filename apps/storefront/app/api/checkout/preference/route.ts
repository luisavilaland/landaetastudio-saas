...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (order.customerEmail !== callerEmail) {
      logger.warn(
        { orderId, callerEmail, orderEmail: order.customerEmail },
        'Email mismatch — IDOR attempt',
      )
    ...
    try {
      ...
      logger.error(
        { error: fetchError.message, orderId, tenantId },
        'MP preference fetch error',
      )
      throw fetchError
    }
    ...
    if (!apiResponse.ok) {
      logger.error(
        { status: apiResponse.status, orderId, tenantId },
        'MP API error',
      )
    ...
    logger.info(
      { preferenceId: responseData.id, useSandbox, orderId, tenantId },
      'Preference created',
    )
    ...
    logger.error(
      { error: error instanceof Error ? error.message : error, orderId },
      'Checkout preference error',
    )
    return NextResponse.json(
