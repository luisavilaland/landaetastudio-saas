    ...
    if (customDomain && customDomain !== existing[0].customDomain) {
      ...
      if (domainExists.length > 0) {
        return errorResponse(
          'El dominio personalizado ya está en uso',
          409,
          'customDomain',
        )
      }
  ...
  try {
    ...
    await withTenantContext(id, async (ctxTx) => {
      ...
      if (productIds.length > 0) {
        ...
        await ctxTx.delete(dbOrders).where(eq(dbOrders.tenantId, id))
        ...
        // Delete products
        await ctxTx.delete(dbProducts).where(eq(dbProducts.tenantId, id))
      }

      // Delete categories
      await ctxTx.delete(dbCategories).where(eq(dbCategories.tenantId, id))

      // Delete customers
      await ctxTx.delete(dbCustomers).where(eq(dbCustomers.tenantId, id))
      ...
      await ctxTx.delete(dbTenants).where(eq(dbTenants.id, id))
    })
